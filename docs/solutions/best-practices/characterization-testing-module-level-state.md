...
- [characterization-testing-rhf-realtime-false-greens.md](characterization-testing-rhf-realtime-false-greens.md) — **sibling learning**: the same mutation-verify-the-guard discipline applied to react-hook-form autosave + realtime-subscription hooks (the A2 cluster), with `fireEvent` / real-timer / supabase-channel mechanics instead of this doc's `resetModules` + fake timers.
