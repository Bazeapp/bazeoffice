-- FASE 4 BIS — gate2_lavoratori: filtri annidati AND/OR.
-- Unica modifica: filtri utente via public.lavoratore_matches_any (dispatcher)
-- invece di lavoratore_matches_filters. Gruppi OR valutati ricorsivamente.
CREATE OR REPLACE FUNCTION public.gate2_lavoratori(p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_search text DEFAULT NULL::text, p_filters jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE sql
