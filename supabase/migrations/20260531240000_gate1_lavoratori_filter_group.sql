-- FASE 4 BIS — gate1_lavoratori: filtri annidati AND/OR.
-- Unica modifica vs versione precedente: la chiamata ai filtri utente usa
-- public.lavoratore_matches_any (dispatcher) invece di lavoratore_matches_filters,
-- così i gruppi OR sono valutati ricorsivamente. Resto invariato.
CREATE OR REPLACE FUNCTION public.gate1_lavoratori(p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_search text DEFAULT NULL::text, p_filters jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE sql
