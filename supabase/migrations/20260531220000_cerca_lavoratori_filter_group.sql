-- FASE 4 BIS — cerca_lavoratori: supporto filtri annidati AND/OR.
-- Aggiunta (additiva) nel WHERE di `eligible`:
--   and (jsonb_typeof(p.filters_value) <> 'object'
--        or public.lavoratore_matches_filter_group(to_jsonb(l), p.filters_value))
-- Quando p_filters è un GRUPPO (oggetto {kind,logic,nodes}) il matcher ricorsivo
-- valuta AND/OR; quando è un array, il fast-path indicizzato resta invariato.
CREATE OR REPLACE FUNCTION public.cerca_lavoratori(p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_search text DEFAULT NULL::text, p_filters jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE sql
...
AS $function$
  ...
  page_rows as (
    ...
    order by stato_lavoratore asc, data_ora_ultima_modifica desc nulls last, creato_il desc nulls last
