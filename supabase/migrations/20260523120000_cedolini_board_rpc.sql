-- Cedolini (Payroll) board + detail RPCs.
-- Replaces the client-side cascade in use-payroll-board.ts (board) and the
-- per-card fan-out in payroll-overview-view.tsx (detail) with two server-side
-- functions that JOIN in the database and return a single payload each.
--
-- Mirrors the style of public.rapporti_lavorativi_board / crm_pipeline_*.
-- Read-only, SECURITY DEFINER, granted to anon + authenticated. Inert until the
-- frontend is switched over to call them.

create index if not exists idx_mesi_lavorati_mese_id
  on public.mesi_lavorati (mese_id);

create index if not exists idx_transazioni_finanziarie_mese_lavorativo_id
  on public.transazioni_finanziarie (mese_lavorativo_id);

create index if not exists idx_pagamenti_transazione_id
  on public.pagamenti (transazione_id);

create index if not exists idx_mesi_calendario_data_inizio
  on public.mesi_calendario (data_inizio);

-- ---------------------------------------------------------------------------
-- Board: all cedolini for a given month (YYYY-MM), enriched with the light
-- related records the kanban cards need. Presenze (166 cols) are intentionally
-- excluded here; the detail RPC loads them on card open.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Detail: one cedolino with its full related records, including the day-by-day
-- presenze (regular + irregular). Returns full rows via to_jsonb.
-- ---------------------------------------------------------------------------
...
as $$
  select case when ml.id is null then null else jsonb_build_object(
    'record', to_jsonb(ml),
    'rapporto', (select to_jsonb(rl) from public.rapporti_lavorativi rl where rl.id = ml.rapporto_lavorativo_id),
    'famiglia', (
      select to_jsonb(f)
