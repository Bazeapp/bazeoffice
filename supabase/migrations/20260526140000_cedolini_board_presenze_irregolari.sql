-- Expose a precomputed `presenzeIrregolari` boolean on the cedolini_board RPC
-- so the payroll kanban card can show the "irregolari" badge without loading
-- the full presenze_mensili row.
--
-- A presenze record is considered "irregolare" when ANY evento_day_1..31 column
-- on the linked presenze_mensili row is non-null and non-empty.
