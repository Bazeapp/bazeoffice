-- Expose the rapporto-level "fee concordata" (richieste_attivazione.fee_concordata)
-- on the cedolino RPCs so the payroll UI can show it next to the totals and
-- compute the application fee as fee_concordata × ore da pagare.
--
-- The column lives on public.richieste_attivazione and is reached via
-- rapporti_lavorativi.richiesta_attivazione_id. Both cedolini_board and
-- cedolino_detail return a slim `richiestaAttivazione` object.
...
as $$
  select case when ml.id is null then null else jsonb_build_object(
    ...
    'richiestaAttivazione', (
      select jsonb_build_object('id', ra.id, 'fee_concordata', ra.fee_concordata)
      from public.richieste_attivazione ra
      where ra.id = (
        select rl.richiesta_attivazione_id
        from public.rapporti_lavorativi rl
        where rl.id = ml.rapporto_lavorativo_id
      )
    )
