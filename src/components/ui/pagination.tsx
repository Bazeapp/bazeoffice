...
function Range({ from, to, total, unit, className, ...props }: RangeProps) {
  return (
    ...
    >
      ...
      di <strong className="text-foreground-strong font-semibold">{total}</strong>
      {unit ? ` ${unit}` : null}
    </div>
  );
}
...
function Pages({ page, pageCount, onChange, siblingCount = 1, className }: PagesProps) {
  ...
  return (
    <div className={cn("flex items-center gap-1", className)}>
      ...
      {items.map((it, i) =>
        it === "…" ? (
          <span key={`gap-${i}`} className="px-1 text-foreground-faint">
            …
          </span>
        ) : (
          <Button
            ...
            className="tabular-nums min-w-(--h-sm)"
          >
            {it}
          </Button>
        )
...
function PerPage({
  ...
  return (
    <div className={cn("flex items-center gap-2 text-foreground-muted", className)}>
      ...
      >
        <SelectTrigger className="h-7 min-w-15 text-xs tabular-nums">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {options.map((o) => (
