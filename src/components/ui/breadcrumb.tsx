    ...
    <>
      <li className={cn("inline-flex items-center gap-1.5", className)} {...props}>
        <Inner
          ...
          className={cn(
            "inline-flex items-center gap-1.5 rounded-sm px-1.5 py-0.5 no-underline",
            current
...
export function BreadcrumbSeparator({
  ...
  return (
    <li aria-hidden className={cn("text-foreground-faint", className)}>
...
export function BreadcrumbEllipsis({ className }: { className?: string }) {
  return (
    <li className={cn("inline-flex items-center", className)}>
      <button
        type="button"
        className="rounded-sm px-1.5 py-0.5 text-foreground-subtle hover:bg-neutral-100"
