...
export function DataTable<TData, TValue>({
  ...
  function openCellPreview(rowId: string, columnId: string, value: unknown) {
    setCellPreview({
      rowId,
      columnId,
        ...
        {row.getVisibleCells().map((cell) => {
          ...
          return (
            <TableCell
              key={cell.id}
              className={cn(
                "group/cell cursor-default select-text",
              ...
              onDoubleClick={() =>
                openCellPreview(row.id, cell.column.id, rawValue)
              }
            >
              <Popover
                open={
                  cellPreview?.rowId === row.id &&
                  cellPreview?.columnId === cell.column.id
                }
                onOpenChange={(open) => {
              ...
              >
                ...
                <PopoverContent
                  side="bottom"
                  align="start"
                  className="max-h-[70vh] p-0 data-open:animate-none data-closed:animate-none duration-0"
                  style={{
                    width: "min(calc(var(--radix-popover-trigger-width) * 2), 46rem)",
                    maxWidth: "90vw",
                  }}
                >
                  <div className="flex items-center justify-between border-b px-3 py-2">
                    <div className="text-xs font-medium">{cell.column.id}</div>
                  </div>
                  <div className="max-h-[calc(70vh-36px)] overflow-auto p-3">
                    <pre className="text-foreground whitespace-pre-wrap break-words text-xs">
                      {cellPreview?.value ?? formatPreviewValue(rawValue)}
                    </pre>
                  </div>
                </PopoverContent>
              </Popover>
  ...
  function renderGroupedRow(row: Row<TData>): React.ReactNode {
    const groupingColumnId = (row as unknown as { groupingColumnId?: string })
      .groupingColumnId;
    const groupingValue = (row as unknown as { groupingValue?: unknown })
      .groupingValue;
